...
function init(){
	...
	$.post("../ajax/estudiosprosp.php?op=selecttema", function(r){ //parametro r son las opciones que devuelve el id
     $("#id_dato").html(r);
...
function reporte(){
    ...
    $url = '../reportes/rptestudios.php?id=' + id_dato;
    target = "_blank";
...
function datos(){
	...
	$.post("../ajax/estudiosprosp.php?op=datos",{id_dato : id_dato}, function(r){ //parametro r son las opciones que devuelve el id
		r = JSON.parse(r);
...
function calcularvalores(){
    ...
    var promedio = ((parseFloat(puntual)+parseFloat(consistente)+parseFloat(completa)+parseFloat(formal))/4).toFixed(2);
...
function listar()
{
	...
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/estudiosprosp.php?op=listar',
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../ajax/estudiosprosp.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(id_ev_e)
{
	$.post("../ajax/estudiosprosp.php?op=mostrar",{id_ev_e : id_ev_e}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);
        ...
        $.post("../ajax/estudiosprosp.php?op=verpreg",{id_preg_ev : id_preg_ev}, function(r){ //parametro r son las opciones que devuelve el id
            r = JSON.parse(r);
