...
$compras= new Estudiosprosp();

$id_ev_e = isset($_POST["id_ev_e"])? limpiarCadena($_POST["id_ev_e"]):"";
...
switch ($_GET["op"]){
    case 'guardaryeditar':
        ...
        if(empty($id_ev_e)){   
            ...
            $rspta= $compras->editar($id_ev_e,$id_dato,$id_preg_ev,$id_valor,$observacion,$puntual,$consistente,$completa,$formal,$valor_ind);
            echo $rspta ? "Información actualizada" : "No se pudo actualizar la información";
        }   
        break;

    case 'mostrar':
        $rspta=$compras->mostrar($id_ev_e);
    ...
    case 'listar':
        ...
        while($reg=$rspta->fetch_object()){ //recorre los registros de la tabla
            $data[]=array(
                "0"=>'<button class="btn btn-warning" onclick="mostrar('.$reg->id_ev_e.')"><i class="fa fa-pencil-square-o"></i></button>',
                "1"=>$reg->contador,
                "2"=>$reg->valor,
                "3"=>$reg->descripcion,
                "4"=>$reg->valoracion,
        ...
        case 'calcularind':
            $rspta=$compras->calcularind($id_ev_e);
            echo ($rspta);
            break;
