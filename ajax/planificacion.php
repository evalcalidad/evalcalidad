...
$compras= new Planificacion();

$id_ev_p = isset($_POST["id_ev_p"])? limpiarCadena($_POST["id_ev_p"]):"";
...
switch ($_GET["op"]){
    case 'guardaryeditar':
        ...
        if(empty($id_ev_p)){   
            ...
            $rspta= $compras->editar($id_ev_p,$id_dato,$id_preg_ev,$id_valor,$observacion,$puntual,$consistente,$completa,$formal,$valor_ind);
            echo $rspta ? "Información actualizada" : "No se pudo actualizar la información";
        }   
        break;

    case 'mostrar':
        $rspta=$compras->mostrar($id_ev_p);
        echo json_encode($rspta);
        break;

     case 'mostrarcriterios':
            $id=$_REQUEST["id"];
            $rspta=$compras->mostrarcriterios($id);
    ...
    case 'listar':
        ...
        while($reg=$rspta->fetch_object()){ //recorre los registros de la tabla
            $data[]=array(
                "0"=>'<button class="btn btn-warning" onclick="mostrar('.$reg->id_ev_p.')"><i class="fa fa-pencil-square-o"></i></button>',
                "1"=>$reg->id_preg_ev,
                "2"=>$reg->valor,
                "3"=>$reg->descripcion,
                "4"=>$reg->valoracion,
        ...
        case 'calcularind':
            $rspta=$compras->calcularind($id_ev_p);
            echo ($rspta);
            break;
